<template>
  <div>
    ...
    <div class="input-group mb-3">
      <button @click="addIngredientField" class="btn btn-secondary"><span class="material-symbols-outlined" style="padding-top: 6px">add</span></button>
    </div>
...
export default {
  ...
  methods: {
    ...
    async postRecipeData(recipeData) {
      ...
      try {
        const response = await fetch('/api/recipes/postCustomRecipe', options);
        if (response.ok) {
          console.log('hat geklappt');
          await this.$router.push('/Rezepte');

        } else {
          console.error('och nööööö');
